...
describe('creating a task from the add-task form', () => {
    ...
    it('sends a child of a server task with its parent id and lane', async () => {
        ...
        expect(JSON.parse(fetcher.mock.calls[0][1].body)).toEqual(createPayload('Child', {
            status: 'doing',
...
describe('importing backup tasks', () => {
    ...
    it('replaces the board with the tasks the server imported', async () => {
        ...
        expect(result).toEqual({ ok: true, queued: false, summary: expect.objectContaining({ importedTasks: 1, replacedTasks: 1 }) });
    ...
    ])('imports the file on this device and queues a $mode import when the server is unavailable', async ({ mode, ids }) => {
        fetcher.mockResolvedValue(jsonResponse({ message: 'Database unavailable.' }, { status: 503 }));
