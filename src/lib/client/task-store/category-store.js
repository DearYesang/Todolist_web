...
export const categories = derived([tasks, categoryCatalog], ([$tasks, $categoryCatalog]) => {
	const hiddenCategoryIds = new Set(
		$categoryCatalog.filter((category) => category.hiddenAt || category.archivedAt).map((category) => category.id)
	);
...
export async function renameCategory(sourceCategory, targetCategory) {
	...
	const changed = rewriteLocalCategory(
		source,
		{ id: source.id ?? null, name: target, color: null, sortOrder: 0, hiddenAt: null, archivedAt: null },
		{ sync: true }
	);
	return {
		ok: true,
		changed,
		message: `${changed}개 작업의 카테고리를 이 기기에서 수정했습니다. 온라인 동기화는 작업 변경으로 재시도됩니다.`
	};
}
...
export async function mergeCategory(sourceCategory, targetCategory) {
	...
	if (
		...
		|| Boolean(source.id && target.id && source.id === target.id)
	) {
	...
	return {
		ok: true,
		changed,
		message: `${changed}개 작업을 이 기기에서 병합했습니다. 온라인 동기화는 작업 변경으로 재시도됩니다.`
	};
}
...
export function assignTaskCategory(taskId, name) {
	...
	const target = categoryName
		? (get(categoryCatalog).find((category) => !category.archivedAt && normalizeCategoryKey(category.name) === key)
			?? null)
		: null;
...
function rewriteLocalCategory(source, target, options) {
	...
	tasks.update((current) => {
		const next = current.map((task) => {
			const isMatch = source.id ? task.categoryId === source.id : normalizeCategoryName(task.category) === sourceName;
			if (!isMatch) {
				return task;
			}
...
function upsertCategoryCatalog(category) {
	categoryCatalog.update((current) =>
		normalizeCategoryCatalog([...current.filter((item) => item.id !== category.id), category])
	);
}
