...
describe('assigning a task category', () => {
	...
	beforeEach(() => {
		...
		vi.stubGlobal(
			'fetch',
			vi.fn(async (/** @type {string} */ _url, /** @type {RequestInit} */ init) => {
...
describe('task versions after a category write', () => {
	...
	beforeEach(() => {
		...
		replaceTasks([
			{ id: TASK_ID, text: 'Read', version: 3, category: '공부', categoryId: SOURCE.id, categoryMeta: SOURCE }
		]);
	...
	it.each([
		[
			'rename',
			() => renameCategory(SOURCE, '국어'),
			{ category: { ...SOURCE, name: '국어' }, updatedTasks: 1 },
			'국어'
		],
		...
		'edits a task after a category %s with the version the server gave it, without a false 409',
		async (_write, write, response, category) => {
			...
			expect(
				requests.filter((request) => request.url === `/api/tasks/${TASK_ID}`).map((request) => request.body)
