...
export function applyServerTaskSnapshot(serverTasks) {
	...
	tasks.update((current) => {
		...
		const busyTasksMissingFromSnapshot = current.filter(
			(task) => isServerId(task.id) && !incomingIds.has(task.id) && hasPendingTaskSync(task.id)
		);
...
function applyServerTaskResult(serverTask) {
	...
	tasks.update((current) =>
		current.map((task) => {
			...
			const next =
				typeof serverTask.version === 'number'
			...
			return subtasks.some((item, index) => item !== next.subtasks[index]) ? { ...next, subtasks } : next;
		})
	);
}
...
function shouldChainLocalChecklistEdit(taskId, subtaskId) {
	return !isServerId(subtaskId) && (resolvedChecklistItemIds.has(subtaskId) || taskSyncChains.has(taskId));
}
...
function buildChecklistDrainMutation(taskId, subtaskId, patch) {
	const itemId = isServerId(subtaskId) ? subtaskId : resolvedChecklistItemIds.get(subtaskId);
	if (itemId) {
		return patch ? { type: 'checklist.patch', taskId, itemId, patch } : { type: 'checklist.delete', taskId, itemId };
	}

	const hasQueuedCreate = loadOfflineQueue().some(
		(mutation) =>
			mutation.type === 'checklist.create' && mutation.taskId === taskId && mutation.localItemId === subtaskId
	);
	if (hasQueuedCreate) {
