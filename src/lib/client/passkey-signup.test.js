...
import { createPasskeySignupContext, getPasskeySignupError, isExpiredVerificationCode } from './passkey-signup.js';
...
describe('passkey sign-up checks', () => {
	...
	it('reports the first missing or stale input, in order', () => {
		expect(getPasskeySignupError(signupInput({ email: '', emailVerificationCode: '' }), NOW)).toBe(
			'이메일을 입력해 주세요.'
		);
		expect(
			getPasskeySignupError(signupInput({ emailVerificationCode: '   ', verificationEmail: 'other@example.com' }), NOW)
		).toBe('이메일 확인 코드를 입력해 주세요.');
		expect(
			getPasskeySignupError(
				signupInput({ verificationEmail: 'other@example.com', verificationExpiresAt: '2000-01-01T00:00:00.000Z' }),
				NOW
			)
		).toBe('현재 이메일로 새 확인 코드를 받아 주세요.');
		expect(getPasskeySignupError(signupInput({ verificationExpiresAt: '2026-09-24T03:00:00.000Z' }), NOW)).toBe(
	...
	it('asks for a recovery code instead of an email code in recovery mode', () => {
		const recovery = {
			...
			verificationEmail: 'other@example.com',
			verificationExpiresAt: '2000-01-01T00:00:00.000Z'
		};

		expect(getPasskeySignupError(signupInput({ ...recovery, recoveryCode: '  ' }), NOW)).toBe(
			'복구 코드를 입력해 주세요.'
		);
		expect(getPasskeySignupError(signupInput({ ...recovery, recoveryCode: 'ABCD-1234' }), NOW)).toBeNull();
		expect(getPasskeySignupError(signupInput({ ...recovery, email: '', recoveryCode: 'ABCD-1234' }), NOW)).toBe(
			'이메일을 입력해 주세요.'
		);
	});
...
describe('passkey sign-up context', () => {
	...
	it('sends the email code, with the name defaulting to the email', () => {
		const context = createPasskeySignupContext(
			signupInput({ emailVerificationCode: ' 123456 ', recoveryCode: 'unused' })
		);
	...
	it('sends the recovery code instead in recovery mode', () => {
		const context = createPasskeySignupContext(
			signupInput({ name: '  사용자 ', isRecoveryMode: true, recoveryCode: ' ABCD-1234 ' })
		);
	...
	it('is read back unchanged by the server', () => {
		...
		expect(
			parsePasskeyRegistrationContext(
				createPasskeySignupContext(signupInput({ isRecoveryMode: true, recoveryCode: 'ABCD-1234' }))
			)
		).toEqual({
			email: 'user@example.com',
