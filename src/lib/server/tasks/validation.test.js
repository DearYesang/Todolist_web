...
describe('server task validation', () => {
	...
	it('reads categoryByName as how to take a null categoryId, not as a task field', () => {
		...
		expect(() => parseUpdateTaskInput({ category: '기획', categoryByName: 'yes' })).toThrow(
			'categoryByName must be a boolean.'
		);
	});
	...
	it('rejects invalid update payloads and date ranges', () => {
		...
		expect(() => parseUpdateTaskInput({ text: 'Bad version', expectedVersion: 0 })).toThrow(
	...
	it('caps titles, categories and checklist texts at the shared limits', () => {
		...
		expect(() => parseCreateTaskInput({ text: 't'.repeat(301), ...dates })).toThrow(
			'Task title must be 300 characters or less.'
		);
		...
		expect(() => parseUpdateChecklistItemInput({ text: 'x'.repeat(501) })).toThrow(
			'Checklist text must be 500 characters or less.'
		);
	});
});
