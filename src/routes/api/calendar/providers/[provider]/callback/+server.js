...
import { CalendarSyncError, completeCalendarProviderAuthorization } from '$lib/server/calendar/provider-sync.js';
import { createCalendarOAuthErrorMessage, createCalendarSyncRedirect } from '$lib/server/calendar/oauth-status.js';
	...
	if (oauthError) {
		throw redirect(
			...
			createCalendarSyncRedirect({
				...
				message: createCalendarOAuthErrorMessage(params.provider, oauthError, url.searchParams.get('error_description'))
			})
		);
	}
