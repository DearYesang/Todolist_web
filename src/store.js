...
export function normalizeDateRange(rawStartDate, rawEndDate) {
    ...
    const startDate = normalizeDateString(rawStartDate) || defaults.startDate;
    const endDate = normalizeDateString(rawEndDate) || defaults.endDate;
...
export function moveTask(taskId, nextStatus) {
    tasks.update((current) => {
        const descendantIds = collectDescendantIds(current, taskId);

        return current.map((task) => {
            if (!descendantIds.has(task.id)) return task;

            if (task.id !== taskId) {
                return { ...task, status: /** @type {TaskStatus} */ (nextStatus) };
            }
...
export function updateTask(taskId, patch) {
    tasks.update((current) => {
        const cascadedStatus = patch.status === 'todo' || patch.status === 'doing' || patch.status === 'done'
            ? patch.status
            : null;
        const descendantIds = cascadedStatus ? collectDescendantIds(current, taskId) : new Set();

        return current.map((task) => {
            const nextTask = { ...task, ...patch };

            if (task.id !== taskId) {
                return cascadedStatus && descendantIds.has(task.id)
                    ? normalizeTask({ ...task, status: cascadedStatus })
                    : task;
            }
