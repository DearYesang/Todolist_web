...
- Calendar sync: Google/Microsoft OAuth 연결, 암호화된 토큰 저장, 수동 provider sync
- Passkey onboarding: 이메일 확인 코드 기반 패스키 생성, 복구 코드, 패스키 로그인, 로그아웃 UI
- Server sync: 인증된 작업 생성/조회/수정/삭제와 체크리스트 생성/수정/삭제
- Offline queue: 실패한 서버 write를 localStorage queue에 저장하고 다음 동기화 때 재시도
...
Calendar provider sync additionally requires `CALENDAR_OAUTH_ENCRYPTION_KEY` plus the relevant Google/Microsoft OAuth client credentials.
...
- Email verification delivery uses `EMAIL_DELIVERY_WEBHOOK_URL` in production; local dev can show preview codes with `EMAIL_VERIFICATION_DEV_CODES=true`.
...
- Google/Microsoft calendar sync is manual for now; background workers, webhooks, and recurring events are still future work.
- Offline writes are queued and retried, but full multi-device conflict resolution is still intentionally simple.

## Roadmap

Near-term:

1. Run production DB migrations and configure OAuth/email delivery secrets.
2. Add route-level integration tests around auth, import replace, offline replay, and calendar sync.
