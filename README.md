...
The domain rules are isolated in `src/lib/shared/task-domain.js`, and the server id rule and the task and view enums that the browser, the API and the database share, and the text length limits the API enforces, live in `src/lib/shared/task-rules.js`. Components read the board from `src/lib/client/task-store.js` and change it only through its commands (create, import, edit, category, filter and view commands); the facade exports its stores read-only and only the names that code outside `src/lib/client/task-store/` uses. That folder holds the task cache, view preference, filters, categories, the per-task sync engine, task creation, import and edits, cross-tab sync, and the server sync that flushes the offline queue and applies the server's tasks, categories and default view. Only that folder adds to the offline queue or flushes it.

On the server, the API routes and the calendar services import the task data layer from `src/lib/server/tasks/repository.js`. It re-exports its sibling modules: `task-repository.js` (task reads, create, partial update and cascade delete), `checklist-repository.js` and `import-repository.js` (backup import and replace). `task-rows.js` holds the task authorization read and other helpers the writers share. Each user's Personal workspace and Inbox board, and the board's default view, are in `src/lib/server/boards/board-provisioning.js`, which the task, category and calendar code and the board preferences route import directly. The task code imports the category code in `src/lib/server/categories/`, never the other way round.

Each API route handler for a signed-in user starts with `requireAuthUser` and, for task writes and imports, the per-user limit in `src/lib/server/security/rate-limit-guard.js`. The other routes are public: email verification, the Better Auth handler under `/api/auth/`, `/api/health`, the calendar cron (behind its bearer secret) and the `.ics` subscription feed (behind the token in its URL). Routes that take a JSON body read it with `readJsonBody`, and routes that answer JSON answer a caught error with `apiErrorResponse`, both in `src/lib/server/http/api-error.js`: a body that is not JSON is a 400, the server's errors (`TaskWriteError`, `RateLimitError`, the calendar and account errors) extend `ApiError` and are answered as `{ message }` with their status and headers (Retry-After on a 429), and any other error is a logged 500. A route that answers only some `ApiError`s names their classes, and the configuration errors (`CalendarTokenConfigurationError`, `CalendarTokenEncryptionError` and `AccountSecurityConfigurationError`, whose messages name environment variables) are answered only by a route that names their class. The OAuth connect and callback routes, which redirect, and the `.ics` feed, which answers plain text, keep their own error handling; email verification checks its declared content length itself and answers the account errors with generic messages before it calls `apiErrorResponse`. `/api/health` and the calendar cron check their bearer secrets with `src/lib/server/security/bearer-secret.js`.

`src/architecture.test.js` reads the imports under `src/` and fails on an import cycle between files or between the folders under `src/lib/server/` (and on any category file importing task code), on a layer import in the wrong direction (`shared` importing `client`, `server` or `components`; `client` importing `server`; `server` importing `client` or `components`), on code outside `src/lib/client/task-store/` (tests included) that reaches a module in that folder without going through `task-store.js`, on production code that reaches a module behind `repository.js` without going through it, on a `task-store.js` export that no production code outside the folder imports, on production code outside the folder other than `src/lib/client/user-scope.js` that imports `offline-write-queue.js`, on `user-scope.js` importing anything from it but its owner, size and clear functions, and on a write to one of the task store's stores outside the folder: the facade exports them read-only, and the test also catches a component's `$tasks = ...` or `bind:value={$filters...}`, which svelte-check lets through. Tests import the module they test directly (outside the task store folder, through the facade) and share their stubs from `src/lib/test-support/`, which only tests may import.

In the browser, `src/lib/client/user-scope.js` holds the per-user state: it caches the signed-in account for offline unlock, `applyUserScope` switches the task store, the offline queue and the link-open state to a user together, and `clearUserLocalData` deletes that user's queue, cached tasks and pending default view when sign-out asks for it. When the owner changes, `src/lib/client/task-store/owner.js` also resets the board state the previous user left (the category catalog, the filters and, from one signed-in user to another, a pending default view). `src/lib/components/App.svelte` calls `applyUserScope` as the session changes, starts a server sync when a user signs in or the browser comes back online, and renders `AppHeader.svelte` (title, view toggle, account, refresh, calendar and backup controls) and `SyncNoticeBanner.svelte` (offline conflicts and sync notices). `src/lib/client/sync-status.js` runs that sync and the header's refresh, and holds what the banner shows (the notice, the conflicts and their actions) and whether the browser is online; the banner and the header read it and call its actions directly. The banner's decisions live in `src/lib/client/offline-conflicts.js`. The view toggle calls `selectView` in `src/lib/client/task-store/view-preference.js`, which saves the chosen view as the account's default and, offline, keeps it for the next sync to send. Backup import and export are in `src/lib/client/backup-transfer.js`, and the pagehide drain and service-worker update reload in `src/lib/client/page-lifecycle.js`. `AuthPanel.svelte` shows `AuthSignInControls.svelte` when signed out and `AuthAccountControls.svelte` (with `PasskeyManager.svelte`) when signed in; their pure helpers are in `src/lib/client/auth-labels.js` and `src/lib/client/passkey-signup.js`.

The Kanban and matrix views (`KanbanBoard.svelte`, `EisenhowerMatrix.svelte`) render each lane as nested `TaskTreeCard.svelte` cards. A card takes only its task, its lane's children and its depth: each board sets the typed context in `src/lib/components/board-context.js` once, with its drag and drop and the callback that opens the detail panel, and cards look up parents, children and subtree links in the task store's `taskIndex`, the lookups over the full task list that `TaskModal.svelte` uses too. The chips and the open-all-links buttons that a card and the detail panel both show are `TaskBadges.svelte` and `TaskLinkActions.svelte` in `src/lib/components/task/`.
