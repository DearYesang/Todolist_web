...
Each API route handler starts with `requireAuthUser` and, for task writes and imports, the per-user limit in `src/lib/server/security/rate-limit-guard.js`. It reads its body with `readJsonBody` and answers a caught error with `apiErrorResponse`, both in `src/lib/server/http/api-error.js`: a body that is not JSON is a 400, the server's errors (`TaskWriteError`, `RateLimitError`, the calendar and account errors) extend `ApiError` and are answered as `{ message }` with their status and headers (Retry-After on a 429), and any other error is a logged 500. A route that answers only some `ApiError`s names their classes. `/api/health` and the calendar cron check their bearer secrets with `src/lib/server/security/bearer-secret.js`.
