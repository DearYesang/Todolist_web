...
The database client is initialized lazily from `$lib/server/db`, so local tests and builds do not require a `DATABASE_URL`.
...
2. Add Better Auth with passkey-first login.
...
5. Ship PWA install/offline basics.
6. Add read-only iCalendar feed.
