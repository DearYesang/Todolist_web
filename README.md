...
`src/architecture.test.js` reads the imports under `src/` and fails on an import cycle, on a layer import in the wrong direction (`shared` importing `client`, `server` or `components`; `client` importing `server`; `server` importing `client` or `components`), on production code that reaches a module in `src/lib/client/task-store/` or behind `repository.js` without going through the facade, and on a write to one of the task store's stores outside `src/lib/client/task-store/`: the facade exports them read-only, and the test also catches a component's `$tasks = ...` or `bind:value={$filters...}`, which svelte-check lets through. Tests import the module they test directly and share their stubs from `src/lib/test-support/`, which only tests may import.
