...
- Offline queue: 실패한 서버 write와 오프라인 JSON import를 사용자별 localStorage queue에 저장하고 다음 동기화 때 재시도
...
- Offline writes and offline JSON imports are queued per user and retried; `409` conflicts are detected, dropped from retry, and surfaced in the UI, but full field-level multi-device merge is still intentionally simple.
...
1. Deploy to Vercel with `todokanban-alpha.vercel.app`, Neon Asia, Resend sandbox delivery, and manual DB migrations.
2. Configure Google Calendar OAuth credentials in production and run a real provider sync smoke test.
3. Add richer field-level multi-device conflict resolution for queued offline writes.
