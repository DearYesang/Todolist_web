...
- JSON import/export supports server-backed append import/export when authenticated, with local cache fallback.
- Two-way calendar provider sync is planned but not implemented
...
2. Add replace import with statement-level atomicity or transaction-capable DB access.
3. Add conflict policy for offline server-backed writes.
4. Add account recovery or verified email before production passkey-only enforcement.
5. Add Google/Microsoft calendar provider sync.
...
- Set `CALENDAR_TOKEN_SECRET` separately from auth secrets before issuing subscription URLs.
