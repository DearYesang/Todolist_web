...
`src/architecture.test.js` reads the imports under `src/` and fails on an import cycle, on a layer import in the wrong direction (`shared` importing `client`, `server` or `components`; `client` importing `server`; `server` importing `client` or `components`), on code outside `src/lib/client/task-store/` (tests included) that reaches a module in that folder without going through `task-store.js`, on production code that reaches a module behind `repository.js` without going through it, on a `task-store.js` export that no production code outside the folder imports, on production code outside the folder that imports from `offline-write-queue.js` anything but its owner, size and clear functions, and on a write to one of the task store's stores outside the folder: the facade exports them read-only, and the test also catches a component's `$tasks = ...` or `bind:value={$filters...}`, which svelte-check lets through. Tests import the module they test directly (outside the task store folder, through the facade) and share their stubs from `src/lib/test-support/`, which only tests may import.
