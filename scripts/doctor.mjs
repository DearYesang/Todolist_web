...
function checkDatabaseUrl() {
	...
	if (!/^postgres(?:ql)?:\/\//.test(value))
		return createCheck('DATABASE_URL', true, 'invalid', 'must be a Postgres URL.');
	return createCheck('DATABASE_URL', true, 'ok', 'configured.');
}

function checkSecret(key, value, required) {
	if (!value)
		return createCheck(key, required, required ? 'missing' : 'optional', required ? 'required.' : 'optional.');
	if (isPlaceholder(value)) return createCheck(key, required, 'placeholder', 'must be replaced.');
	if (value.length < MIN_SECRET_LENGTH)
		return createCheck(key, required, 'weak', `should be at least ${MIN_SECRET_LENGTH} characters.`);
...
function checkUrl(key, value, required) {
	...
	try {
		...
		if (!['http:', 'https:'].includes(parsed.protocol))
			return createCheck(key, required, 'invalid', 'must use http or https.');
		return createCheck(key, required, 'ok', 'configured.');
	} catch {
...
function checkUrlList(key, value, required) {
	...
	const invalid = value
		...
		.map((item) => item.trim())
		.filter(Boolean)
		.some((item) => checkUrl(key, item, true).status !== 'ok');
...
function checkEmailList(key, value, required) {
	...
	return createCheck(
		...
		required,
		invalid ? 'invalid' : 'ok',
		invalid ? 'must contain comma-separated email addresses.' : 'configured.'
	);
}
...
function checkPositiveInteger(key, value, required) {
	...
	if (!Number.isInteger(number) || number <= 0)
		return createCheck(key, required, 'invalid', 'must be a positive integer.');
	return createCheck(key, required, 'ok', 'configured.');
}
