...
- Latest production merges (2026-07-06 ~ 07-11): #54 offline-sync race-condition fixes, #55 dependency security update (`npm audit` 0), #56 server write-path hardening (atomic batches, recursive-CTE cascade delete, partial PATCH, per-user rate limits), #57 Playwright E2E in CI on Chromium + iPhone WebKit, #61 task search and due-date highlighting, #62 pointer-based drag and drop with touch support across Kanban/Eisenhower, #63 stronger pointer-DnD E2E persistence assertions, plus Dependabot action bumps.
- CI went red after 2026-07-11 because new `npm audit` advisories (4 moderate, 4 high) fail the Audit step that runs before tests. A lockfile-only `npm audit fix` chore PR clears them and must land before any feature PR.
- Open-all checklist links (this document's 2026-09-24 update) still needs a manual check on the Vercel preview in the installed Brave PWA before it reaches production; see Recommended Next Order.
...
| Task core | Kanban with hierarchy, checklist, filters, import/export, server sync, category management, and ergonomic schedule input. | Kanban, task hierarchy, checklist, filters, DB-backed category entity v2, category rename/merge/delete, category colors, sorting, hide/archive, category counts, rule-based category suggestions while adding/editing tasks, JSON import/export, server CRUD, checklist CRUD, cascade delete, optimistic writes, offline queue, and a reusable visual date-range picker for task creation/detail editing are implemented on the current branch. Partial task response merge now preserves parent links during checklist sync. Task update/delete conflicts now offer apply-local or keep-server actions. Free-text search across titles/categories/checklists and overdue/due-today date-tag highlighting are implemented; category v2 is live in production. Checklist links are parsed by one shared module (`src/lib/shared/task-links.js`) that keeps trailing punctuation, typographic quotes and unbalanced brackets such as `기관(https://…)` out of the link and accepts only http(s). Cards and the detail panel offer `🔗 모두 열기 (N)` and `🔗 하위 포함 모두 열기 (M)` (checked items included; the subtree walks every descendant, not just visible ones), with a bottom panel for confirmation above 10 links, a 20-per-click cap, and a pop-up-blocked fallback. | Checklist/import conflicts are still conservative and report-first. LLM-based category automation remains deferred until the manual taxonomy feels stable. Real pop-up blocking cannot be exercised in Playwright, so the open-all fallback is verified by hand. The link parser keeps unencoded Hangul in query values, fragments, IDN hosts and inner path segments, but a Hangul run that ends a URL (`/wiki/대한민국`) is cut off, because it cannot be told apart from glued prose (`https://kss.or.kr/에서`); paste such URLs percent-encoded. |
| Views | Kanban and Gantt first; add planning views as needed. | Kanban, Gantt centered on today, Gantt task-list checklist previews with completion toggles, and Eisenhower matrix are implemented. Cards move by pointer drag on mouse and touch (long-press) in Kanban and Eisenhower. Matrix hides completed tasks by default with an opt-in toggle. The last selected view is persisted per account in `boards.default_view`; `localStorage` remains the offline fallback. | Verify cross-device default view behavior on physical devices. Watch whether Gantt checklist toggles need an undo affordance after daily use. |
...
1. Verify open-all links by hand on the Vercel preview before merging: in the installed Brave PWA, `외부 정보 탐색 → 하위 포함 모두 열기` should open one tab and show the partial panel with the pop-up-blocked icon in the app title bar; after 'always allow pop-ups' every link should open as a Brave tab with no `)` in any address. Also check Tab + Enter activation, and iPhone Safari / the home-screen app (expect the link-list fallback there).
...
5. Small refactor follow-ups, one PR each: shared blob download helper (also fixes UTC-dated backup filenames), shared local-date and Gantt geometry modules, shared client HTTP helpers, and an IME composition guard (needs a manual Korean-IME check).
