...
4. Technical hardening batch (one PR): client fetch timeouts + `retry-after` honoring (in the shared `src/lib/client/http.js`), `rate_limit_buckets` and soft-deleted-task purge on the existing cron, single-JOIN task authorization query.
