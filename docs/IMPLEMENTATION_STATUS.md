...
| Security and operations | Tight `.gitignore`, generated secrets, env runbooks, no secret commits. | `.env`-style files are ignored, secret generator and production doctor exist, strict health validates required env/passkey origin values and blocks preview email codes in production, OAuth tokens are encrypted at rest, iCal tokens are HMAC-hashed, baseline CSP/security headers are added, and public docs use placeholder emails. | Add periodic dependency/security review and rotate secrets if any machine or deployment access changes. |
