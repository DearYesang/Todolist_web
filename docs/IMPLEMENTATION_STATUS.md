...
| Security and operations | Tight `.gitignore`, generated secrets, env runbooks, no secret commits, public-repo hygiene. | `.env`-style files and local analysis artifacts are ignored, secret generator and production doctor exist, strict health validates required env/passkey origin values and blocks preview email codes in production, OAuth tokens are encrypted at rest, iCal tokens are HMAC-hashed, SvelteKit-owned CSP hashes page bootstrap scripts, fallback API security headers are added, iCal feed `lastUsedAt` writes are throttled, iCal token/IP requests have best-effort rate limiting, logout warns before deleting unsynced offline changes, GitHub secret scanning/push protection are enabled, CI runs `npm audit`, and Dependabot monitors npm/GitHub Actions. | Rotate secrets if any machine or deployment access changes. Keep reviewing dependency PRs and security alerts. |
